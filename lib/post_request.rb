...
class PostRequest
	class << self
		...
		def non_utility_service_type(title)
		  	response = HTTParty.post( "http://izkh.ru/api/1.0/nonutilityservicetype?auth_token=#{Auth.get}",
		    	:body => { :non_utility_service_type => { title: title }}.to_json,
		    	:headers => {'Content-Type' => 'application/json'})
		end

		def servicetype(title)
		  	response = HTTParty.post( "http://izkh.ru/api/1.0/servicetype?auth_token=#{Auth.get}",
		...
		def tariff_template(vendor_id, service_type_id)
			...
			response = HTTParty.post( "http://izkh.ru/api/1.0/tariff_template?auth_token=#{Auth.get}",
		...
		def freelancer(freelance_category_id, description, phone, title, name)
		  	response = HTTParty.post( "http://izkh.ru/api/1.0/freelancer/create?auth_token=#{Auth.get}",
		...
		def payload(data)
			response = HTTParty.post( "http://izkh.ru/api/1.0/account/autoset?auth_token=#{Auth.get}",
