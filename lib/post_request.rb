...
class PostRequest
	...
	def self.servicetype(title)
	  	response = HTTParty.post( "https://izkh.ru/api/1.0/nonutilityservicetype?auth_token=#{Auth.get}",
	    	:body => { :servicetype => { title: title }}.to_json,
	    	:headers => {'Content-Type' => 'application/json'})
	end
	...
	def self.precinct(ovd, ovd_town, ovd_street, ovd_house, ovd_telnumber, surname, name, middlename)
	...
	def self.precinct_territory(precinct_id, street, house)
