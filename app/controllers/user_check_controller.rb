class UserCheckController < ApplicationController
  def address
    ...
    @address_check = AddressRange.where("city = ? AND street = ? AND building = ? AND apartment = ? AND vendor_id = ?", params[:city], params[:street], params[:building], params[:apartment], @vendor.id).first
