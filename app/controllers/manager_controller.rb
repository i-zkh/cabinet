...
class ManagerController < ApplicationController
  ...
  def import
    ...
    if params.has_key?(:file) && vendor
      ...
      filename = "report/#{DateTime.now.year}-#{DateTime.now.month}/" + "#{vendor.id}" + "#{File.extname("#{params[:file].original_filename}")}"
  ...
  def open_spreadsheet(filename, vendor_id)
    begin
      ...
      when ".txt"          then Getter.new(Txt.new(filename, vendor_id)).input
      when ".xls", ".xlsx" then Getter.new(Xls.new(filename, vendor_id)).input
      when ".dbf"          then Getter.new(Dbf.new(filename, vendor_id)).input
      when ".ods"          then Getter.new(Ods.new(filename, vendor_id)).input
      else
